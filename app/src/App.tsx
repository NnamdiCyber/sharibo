...
// The shape `generateProof` resolves to (proof + the public signals the
// contract needs). Captured across a failed claim so a retry can reuse it
// instead of re-running the ~10s proving step.
...
export default function App() {
  ...
  async function doClaim() {
    ...
    try {
      ...
      let generated: GeneratedProof | null = null;
      try {
  ...
  // Re-runs a failed claim WITHOUT re-proving: the proof generated by the
  // original doClaim is still valid for this round, so we reuse it. Before
  // blindly re-submitting, we re-read chain state to check whether the
  // previous attempt actually landed (a submit can fail to report success
  // even when it was included) — if it did, we treat it as success.
  async function retryClaimWithProof(g: GeneratedProof) {
    if (!admin || circleId === null) return;
    setFailure(null);
    setClaimResult(null);
    setRejection(null);
    ...
    try {
      ...
      const { connect, claim, getCircle, hasClaimed } = await import("@sharibo/client");
      const adminClient = await connect(NETWORK, admin);

      // Re-read chain state first — never retry a submitted tx blindly.
      const alreadyClaimed = await hasClaimed(adminClient, circleId, g.nullifierHash);
      if (alreadyClaimed) {
        const circle = await getCircle(adminClient, circleId);
        setProof(g.proof);
        setNullifierHash(g.nullifierHash);
        setClaimResult({ recipient: "—", hash: "", proofDurationMs: 0 });
        setNullifierClaimed(true);
        setPot(circle.pot);
        setRound(circle.round);
        return;
      }

      setBusy("Funding a fresh, unlinked recipient and re-submitting…");
      const recipient = Keypair.random();
      await fundWithFriendbot(recipient.publicKey());
      const { hash } = await claim(adminClient, {
        circleId,
      ...
      setNullifierClaimed(await hasClaimed(adminClient, circleId, g.nullifierHash));
